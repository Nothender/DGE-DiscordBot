...
namespace DiscordGameEngine.Misc.BetaTesting
{
    public static class BetaTestingPointsCounter
    {
        ...
        private static string filepath = Core.Core.pathToSavedData + filename;
        ...
        public static List<UserPointsPair> GetUsersPoints()
        {
            ...
            foreach (string line in File.ReadAllLines(filepath))
            {
                ...
                users.Add(new UserPointsPair(ulong.Parse(lineSplit[0]), int.Parse(lineSplit[1])));
            }
            return users;
        }
