...
namespace DiscordGameEngine.ProgramModules
{
    public abstract class ProgramModule
    {
        ...
        {
            DiscordGameEngineBot.OnShutdown += SaveProgramsData;
        }
        ...
        public bool AddChannel(ulong channelId)
        {
            ...
            interactionChannels.Add(DiscordGameEngineBot._client.GetChannel(channelId) as ISocketMessageChannel);
        ...
        public static void SaveProgramsData(object sender, EventArgs e)
        {
            ...
            File.WriteAllText(Core.Core.pathToSavedData + SaveFileName, JsonSerializer.Serialize(programsData));
        }
        ...
        internal static void RestoreSavedPrograms()
        {
            ...
            loaded = true;
            DiscordGameEngineBot.DGELogger.Log("Restored saved programs", EnderEngine.Logger.LogLevel.INFO);
        }

        private static List<ProgramData> LoadProgramsData()
        {
            if (File.Exists(Core.Core.pathToSavedData + SaveFileName))
                return JsonSerializer.Deserialize<List<ProgramData>>(File.ReadAllText(Core.Core.pathToSavedData + SaveFileName));
            return new List<ProgramData>();
        }

        internal static void DeleteProgramsSaveFile()
        {
            File.Delete(Core.Core.pathToSavedData + SaveFileName);
        }
