...
namespace DiscordGameEngine.UI.Feedback
{
    public static class UserFeedbackHandler
    {
        ...
        private static string filepath = Core.Core.PathToStorage + filename;
        private static string reportIdsFilepath = Core.Core.PathToStorage + reportIdsFilename;

        private static int linesCounter;

        internal static ulong feedbackChannelId; // Used to store the channel Id loaded from the infos file before the bot is connected
        internal static ISocketMessageChannel feedbackChannel;
        ...
        public static bool SendFeedback(FeedbackInfo feedbackInfo)
        {
            ...
            {
                DiscordGameEngineBot.DGELogger.Log($"Report {feedbackInfo.reportId} was skipped (already sent)", Logger.LogLevel.INFO);
                return true;
            }
            ...
            DiscordGameEngineBot.DGELogger.Log($"new FeedbackReport was created (id: {feedbackInfo.reportId}, BeginLineIndex: {linesCounter} EndLineIndex: {linesEnd})", Logger.LogLevel.INFO);
            return false;
        }
