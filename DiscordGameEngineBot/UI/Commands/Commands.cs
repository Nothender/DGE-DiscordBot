...
namespace DiscordGameEngine.UI.Commands
{
    ...
    public class Commands : ModuleBase<SocketCommandContext>
    {
        ...
        {
            await ReplyAsync($"Pong : the latency is {DiscordGameEngineBot._client.Latency}ms");
        }
        ...
        public async Task Help(string valueName = null)
        {
            ...
            if (valueName is null)
            {
                ...
                DiscordGameEngineBot._commands.Modules.ToList().ForEach(m => embed.AddField(
                    m.Name,
            ...
            else
            {
                ...
                List<ModuleInfo> modules = DiscordGameEngineBot._commands.Modules.ToList();
                List<CommandInfo> commands = DiscordGameEngineBot._commands.Commands.ToList();

                if (modules.Any(m => m.Name.ToLower() == valueName))
                {
                    ModuleInfo module = modules.Find(m => m.Name.ToLower() == valueName);
                    module.Commands.ToList().ForEach(c => { embed.AddField($"{c.Name} {(c.Aliases.Count == 1 ? "" : $"({string.Join(", ", c.Aliases.ToArray(), 1, c.Aliases.Count - 1)})")}",
